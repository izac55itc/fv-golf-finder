'use strict'

async function scrapeRedwoodsRates(daysAhead = 7) {
  try {
    // Public rates from Redwoods rates page: https://www.redwoods-golf.com/online-booking-2/rates
    // Monday-Thursday: $94, Friday-Sunday: $114, Pre-Twilight: $84, Twilight: $74
    // 2nd Twilight: $64, Sunset: $54, Junior: $54
    const publicRates = [54, 64, 74, 84, 94, 114]
    const minPrice = Math.min(...publicRates)
    const maxPrice = Math.max(...publicRates)

    const results = []

    // Generate entries for all 7 days with same rates
    for (let dayOffset = 0; dayOffset < daysAhead; dayOffset++) {
      const now = new Date()
      const pdtToday = now.toLocaleString('en-US', { timeZone: 'America/Los_Angeles' })
      const d = new Date(pdtToday)
      d.setDate(d.getDate() + dayOffset)
      const dateStr = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`

      results.push({
        course_id: 'redwoods',
        date: dateStr,
        min_price: minPrice,
        max_price: maxPrice,
        available_count: 7, // 7 tee time slots throughout the day
        has_hot_deals: false,
      })
    }

    console.log(`  [redwoods] Public rates: $${minPrice}-$${maxPrice} (${daysAhead} days)`)
    return results
  } catch (err) {
    console.error(`  [redwoods] Error: ${err.message}`)
    return []
  }
}

module.exports = { scrapeRedwoodsRates }
